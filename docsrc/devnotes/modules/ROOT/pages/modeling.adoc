...
== Model ChannelStatisticsGroup
[source,sh]
----
bin/rails g scaffold ChannelStatisticsGroup \
  name:string \
  --no-stylesheets --no-javascripts --no-helper --no-decorator
----
...
[source,sh]
----
bin/rails g migration CreateJoinTable ChannelStatisticsGroupEscalationLevel \
  channel_statistics_group escalation_level
----
