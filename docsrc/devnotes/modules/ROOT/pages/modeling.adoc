...
[source,sh]
----
bin/rails g migration CreateJoinTable ChannelStatisticGroupEscalationLevel \
  channel_statistic_group escalation_level
----

[source,ruby]
----
def change
  create_join_table :channel_statistic_group, :escalation_level do |t|
    t.index [:notification_group_id, :wobauth_user_id], name: 'idx_notificationgroup_user'
    t.index [:wobauth_user_id, :notification_group_id], name: 'idx_user_notificationgroup'
  end
end
