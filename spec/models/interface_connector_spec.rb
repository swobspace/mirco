...
RSpec.describe InterfaceConnector, type: :model do
  ...
  it { is_expected.to validate_presence_of(:interface_connector_id) }
  ...
  describe "#to_s" do
    it { expect(interface_connector.to_s).to match('BAR in (tcp:5555)') }
  end

  describe "#direction" do
    it { expect(interface_connector.direction).to eq('in') }
  end

  describe "#proto" do
    it { expect(interface_connector.proto).to eq('tcp') }
  end
