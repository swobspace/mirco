...
class IsCurrentComponent < ViewComponent::Base
  def initialize(item:, attr:, grace_period: Mirco::grace_period, relative: false, icon: true)
    @item = item
    @attr = attr
  ...
  def message
    if !icon
      ...
      Mirco::States.flag(Mirco::States::WARNING)
    elsif to_check < grace_period.before(Time.current)
      Mirco::States.flag(Mirco::States::WARNING) + " #{show_time}"
    else
      Mirco::States.flag(Mirco::States::OK) + " #{show_time}"
    end
  end
